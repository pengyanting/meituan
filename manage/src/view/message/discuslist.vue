    ...
    <el-row>
        ...
        <el-col :span="24">
            <el-button-group class="navBtn">
                ...
                <el-button type="primary"
                           @click="handleAdd"><i class="el-icon-plus el-icon--left"></i>新建消息</el-button>
            </el-button-group>
        </el-col>
        <!--表格-->
        <el-col :span="24">
...
export default {
    ...
    methods: {
        handleSearch() {
            var vm = this;
            var sql="select * from discus where 1=1 "
            if(vm.formInline.username){
                sql+="and username like '%"+vm.formInline.username+"%' "
            }
             if(vm.formInline.time){
                sql+="and time='"+vm.formInline.time+"' "
            }
            this.$store.dispatch('LOAD', true);
            axios.post(this.$root._data.apiUrl + "manage/selectmessage.php",qs.stringify({
                sql:sql
            })).then(function (res) {
                    setTimeout(() => {
        ...
        handleAdd() {
            this.$router.push({ path: '/message/addMessage.php' })
        },
